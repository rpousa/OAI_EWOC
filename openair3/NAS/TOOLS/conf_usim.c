...
void gen_usim_data(usim_data_conf_t *u, usim_data_t *usim_data,
  ...
  for (int i = 0; i < user_plmns->operators.size; i++) {
    ...
    usim_pnn_t *pnn = usim_data->pnn + i;
    pnn->fullname.type = USIM_PNN_FULLNAME_TAG;
    pnn->fullname.length = min(sizeof(pnn->fullname.value) - 1, strlen(record.fullname));
    memcpy(pnn->fullname.value, record.fullname + 1, pnn->fullname.length);
    pnn->fullname.value[pnn->fullname.length] = 0;
    pnn->shortname.type = USIM_PNN_SHORTNAME_TAG;
    pnn->shortname.length = min(sizeof(pnn->shortname.value) - 1, strlen(record.shortname));
    memcpy(pnn->shortname.value, record.shortname + 1, pnn->shortname.length);
    pnn->shortname.value[pnn->shortname.length] = 0;
    usim_data->opl[i] = (usim_opl_t){record.plmn, record.tac_start, record.tac_end, i};
  }
